...
withShader :: ShaderType -> BS.ByteString -> (ShaderId -> IO r) -> IO r
withShader shaderType shaderSource f =
  let shaderTypeGL Vertex = GL_VERTEX_SHADER
      shaderTypeGL Fragment = GL_FRAGMENT_SHADER
  in
  bracket (checkGLErrors ( glCreateShader (shaderTypeGL shaderType)))
   -- prevent leak
   glDeleteShader
   (\shaderId -> do
    BS.useAsCString shaderSource
        (\c_string -> let gl_string = castPtr c_string :: Ptr GLchar
    ...
    if compileStatus == GL_TRUE
        ...
        f shaderId
            ...
            if infoLogLength == 0
            then errors >>= (\es -> throw (ShaderProgramCompilationFailed "No information log" es))
            else do
                infoLog <- getInfoLog
                putStrLn ("infoLog: " ++ infoLog)
                errors >>= (\es -> throw (ShaderProgramCompilationFailed infoLog es))
    )
